  ...
  return (
    <>
      <div className="flex flex-col gap-4">
        ...
        {/* Onboarding: explicit Save & Continue button */}
        {nextPath ? (
          <button
            type="button"
            onClick={handleSave}
            disabled={status === "saving"}
            className="w-full bg-amber-500 hover:bg-amber-400 disabled:opacity-40 disabled:cursor-not-allowed text-zinc-950 font-bold text-base rounded-xl py-4 transition-colors min-h-[56px] flex items-center justify-center gap-2"
          >
            {status === "saving" ? (
              <>
          ...
          /* Normal: quiet auto-save status */
          status !== "idle" && (
            <p className={`text-xs text-right ${status === "error" ? "text-red-400" : status === "saved" ? "text-green-400" : "text-zinc-400"}`}>
