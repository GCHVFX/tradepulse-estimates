-- Contractor-owned pricing, Phase 2 slice 3B.
-- See specs/contractor-owned-pricing.md and the Phase 2 audit.
--
-- Replaces only public.tpe_save_contractor_pricing, from the function body
-- already live in production (verified against pg_get_functiondef()
-- immediately before writing this migration). The table already has
-- tpe_estimate_items.taxable boolean not null default true; this function was
-- simply not passing it through. Three additions only, nothing else touched:
-- taxable added to the INSERT column list, its incoming value read as
-- coalesce((e ->> 'taxable')::boolean, true) so a row from an old deployed
-- client (which never sends taxable) still persists true, and 'taxable',
-- taxable added to each row this function returns.
...
begin
  ...
  insert into public.tpe_estimate_items (
    ...
    line_total, markup_percent, display_order, taxable
  )
  select
    p_estimate_id,
    e ->> 'description',
