...
Updated: 2026-09-02 22:24 PT (Trade-tabs scroll affordance fixed on branch `fix/trade-tabs-scroll-affordance`, committed, **not merged -- explicitly held for Greg's manual review**, same as the hero-crop-v2 fix on its own sibling branch `fix/mobile-hero-photo-crop-v2` (also committed, not merged, cut from the same point). See the entry below for the fade-out affordance and why. Unrelated to the mobile-nav/legal-pages/CSV-import/trade-tabs-overflow/hero-crop-v1 work further below, all of which is already merged to `main` and confirmed live in production.)

## Trade tabs scroll affordance: rest state looked broken, not scrollable (2026-09-02 22:24 PT)

**Status:** implemented on new branch `fix/trade-tabs-scroll-affordance`,
cut from `main` at `d2b8605`. Committed. **Not merged -- do not merge
without Greg reviewing this one specifically.**
...
**Exact next action:** Greg reviews the rest-state screenshots (sent)
and the live branch, then decides whether to merge. Not merged yet, on
purpose.
